...
public static class ANSHDbContextOptionsExtensions {
    ...
    /// <param name="anshDbparameters">参数</param>
    /// <param name="anshDbConnection">数据库连接对象</param>
    public static IQueryable<TEntity> FromSql<TEntity> (this DbSet<TEntity> source, string sql, List<ANSHDbParameter> anshDbparameters, ANSHDbConnection anshDbConnection) where TEntity : class {
        using (var cmd = anshDbConnection.DbConnection.CreateCommand ()) {
            var dbParameters = anshDbConnection.CreateDbParameter (cmd, anshDbparameters);
            return source.FromSql (sql, dbParameters?.ToArray ());
        }
    }
    ...
    /// <param name="anshDbParameters">参数</param>
    /// <param name="anshDbConnection">数据库连接对象</param>
    public static IQueryable<TEntity> FromSql<TEntity> (this DbQuery<TEntity> source, string sql, List<ANSHDbParameter> anshDbParameters, ANSHDbConnection anshDbConnection) where TEntity : class {
        using (var cmd = anshDbConnection.DbConnection.CreateCommand ()) {
            var dbParameters = anshDbConnection.CreateDbParameter (cmd, anshDbParameters);
            return source.FromSql (sql, dbParameters?.ToArray ());
        }
